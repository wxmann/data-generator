...
class ConfigTest(unittest.TestCase):
    ...
    def test_funcsetting_args_should_generate_value(self):
        dependencies = None
        funcsetting = config.FunctionSetting(self.dummyfunc, dependencies, 1, 2)
        self.assertEqual(funcsetting.generatevalue(), 3)

    def test_funcsetting_args_should_generate_value_withgenerator(self):
        dependencies = None
        funcsetting = config.FunctionSetting(self.dummygen, dependencies, 1, 2)
        self.assertEqual(funcsetting.generatevalue(), 3)
    ...
    def test_funcsetting_kwargs_should_generate_value(self):
        dependencies = None
        funcsetting = config.FunctionSetting(self.dummyfunc, dependencies, x=1, y=2)
        self.assertEqual(funcsetting.generatevalue(), 3)

    def test_funcsetting_kwargs_should_generate_value_withgenerator(self):
        dependencies = None
        funcsetting = config.FunctionSetting(self.dummygen, dependencies, x=1, y=2)
        self.assertEqual(funcsetting.generatevalue(), 3)
        self.assertEqual(funcsetting.generatevalue(), 4)
        self.assertEqual(funcsetting.generatevalue(), 5)
    ...
    def test_funcsetting_externalargs_should_generate_value(self):
        ...
        funcsetting = config.FunctionSetting(self.dummyfunc, dependencies)
    ...
    def test_funcsetting_mixargskwargs_should_generate_value(self):
        dependencies = None
        funcsetting = config.FunctionSetting(self.dummyfunc, dependencies, 1, y=2)
        self.assertEqual(funcsetting.generatevalue(), 3)

    def test_funcsetting_someexternalargs_should_generate_value(self):
        dependencies = ['Column_A']
        funcsetting = config.FunctionSetting(self.dummyfunc, dependencies, 1)
        self.assertEqual(funcsetting.generatevalue(2), 3)

    def test_funcsetting_someexternalkwargs_should_generate_value(self):
        dependencies = ['Column_A']
        funcsetting = config.FunctionSetting(self.dummyfunc, dependencies, x=1)
    ...
    def test_funcsetting_same_attributes_eq(self):
        dependencies = ['A']
        setting1 = config.FunctionSetting(self.dummyfunc, dependencies, 1, 2)
        setting2 = config.FunctionSetting(self.dummyfunc, dependencies, 1, 2)
        self.assertEqual(setting1, setting2)

    def test_funcsetting_diff_attributes_noteq(self):
        dependencies = ['A']
        setting1 = config.FunctionSetting(self.dummyfunc, dependencies, 1, 2)
        setting2 = config.FunctionSetting(self.dummyfunc, dependencies, 2, 3)
        self.assertNotEqual(setting1, setting2)

    def test_should_set_and_get_funcsetting_no_dependencies(self):
        dependencies = None
        funcsetting_expected = config.FunctionSetting(self.dummyfunc, dependencies, 1, 2)
